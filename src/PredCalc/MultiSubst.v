...
Section syntax.
  ...
  Definition vtmap_fvars m : gset variable :=
    ⋃ (term_fvars ∘ snd <$> map_to_list m).

  Lemma elem_of_vtmap_fvars x m :
    x ∈ vtmap_fvars m → ∃ y t, m !! y = Some t ∧ x ∈ term_fvars t.
  Proof with auto.
    intros. unfold vtmap_fvars in H. apply elem_of_union_list in H as [fvars []].
  ...
  Lemma not_elem_of_vtmap_fvars x m :
    x ∉ vtmap_fvars m → (∀ y t, m !! y = Some t → x ∉ term_fvars t).
  Proof with auto.
    intros. unfold vtmap_fvars in H. unfold not in H. intros contra.
  ...
  Definition quant_msubst_fvars y A m :=
    (formula_fvars A ∖ {[y]}) ∪ vtmap_fvars m ∪ dom m.
  ...
  Local Notation "A [ [ xs \ ts ] ]" := (msubst A (to_vtmap xs ts))
                                          (in custom formula at level 74, left associativity,
                                              xs custom var_seq,
                                              ts custom term_seq) : refiney_scope.
  ...
  Proof with auto.
    intros H.
    destruct af...
    - simpl in *. do 2 rewrite msubst_term_id by set_solver...
    - simpl in *. rewrite msubst_term_id by set_solver...
  ...
  Proof with auto.
    ...
    1-2: repeat rewrite <- msubst_term_delete_non_free by set_solver...
    f_equal. induction args... simpl. f_equal.
  ...
  Proof with auto.
    ...
    - destruct (to_vtmap xs ts !! x) eqn:E.
      + apply elem_of_dom_2 in E. unfold to_vtmap in E.
        rewrite dom_list_to_map_zip_L in E... set_solver.
      + apply not_elem_of_dom_2 in E. unfold to_vtmap in E.
  ...
  Lemma msubst_af_non_free af xs ts `{!OfSameLength xs ts} :
    ...
    msubst_af af (to_vtmap xs ts) = af.
  Proof with auto.
    intros. destruct af; simpl in *...
    1-2: repeat rewrite msubst_term_non_free by set_solver...
  ...
  Lemma fvars_msubst_term_superset t xs ts `{!OfSameLength xs ts} :
    term_fvars (msubst_term t (to_vtmap xs ts)) ⊆ term_fvars t ∪ ⋃ (term_fvars <$> ts).
  Proof with auto.
      ...
      + apply elem_of_list_lookup in e as (i&?). destruct (lookup_of_same_length_l ts H0) as (t&?).
        * replace (to_vtmap xs ts !! x) with (Some t).
          ...
          -- symmetry. unfold to_vtmap. apply lookup_list_to_map_zip_Some...
             ...
             lia.
      + replace (to_vtmap xs ts !! x) with (@None term); [set_solver|].
        symmetry. unfold to_vtmap. apply lookup_list_to_map_zip_None...
  ...
  Proof with auto.
    ...
    - generalize_fresh_var_for_msubst x A (to_vtmap xs ts) as x'.
  ...
  Proof with auto.
    ...
    - simpl. destruct (decide (x ∈ xs1 ∨ x ∈ xs2)).
      2:{ apply Decidable.not_or in n as [].
          ...
          2:{ symmetry. unfold to_vtmap. apply lookup_list_to_map_zip_None... }
          simpl. replace (to_vtmap xs2 ts !! x) with (@None term)...
          symmetry. unfold to_vtmap. apply lookup_list_to_map_zip_None... }
      destruct o.
      + pose proof (Hdisjoint x H).
        ...
        2:{ symmetry. unfold to_vtmap. apply lookup_list_to_map_zip_Some;
            ...
            - intros. apply NoDup_lookup with (i:=i) in H4... lia. }
        simpl. replace (to_vtmap xs2 ts !! x') with (Some t).
        2:{ symmetry. unfold to_vtmap. apply lookup_list_to_map_zip_Some;
            [typeclasses eauto|]. exists i. split_and!...
            intros. apply NoDup_lookup with (i:=i) in H4... lia. }
        replace (to_vtmap xs1 ts !! x) with (Some t)...
        symmetry. unfold to_vtmap. apply lookup_list_to_map_zip_Some;
          [typeclasses eauto|]. exists i. split_and!...
        intros. apply NoDup_lookup with (i:=i) in H4... lia.
  ...
  Proof with auto.
    ...
    - simpl. destruct (decide (x ∈ xs1 ∨ x ∈ xs2)).
      2:{ apply Decidable.not_or in n as [].
          simpl. replace (to_vtmap (xs1 ++ xs2) (ts1 ++ ts2) !! x) with (@None term).
          2:{ symmetry. unfold to_vtmap. apply lookup_list_to_map_zip_None...
              - typeclasses eauto.
              - set_solver. }
          replace (to_vtmap xs1 ts1 !! x) with (@None term).
          2:{ symmetry. unfold to_vtmap. apply lookup_list_to_map_zip_None... }
          simpl. replace (to_vtmap xs2 ts2 !! x) with (@None term)...
          symmetry. unfold to_vtmap. apply lookup_list_to_map_zip_None... }
      destruct o.
      + pose proof (Hdisjoint x H).
        apply elem_of_list_lookup in H as [i ?].
        destruct (lookup_of_same_length_l ts1 H) as [t ?].
        replace (to_vtmap (xs1 ++ xs2) (ts1 ++ ts2) !! x) with (Some t).
        2:{ symmetry. unfold to_vtmap. apply lookup_list_to_map_zip_Some;
            [typeclasses eauto|]. exists i. split_and!...
        ...
        simpl. replace (to_vtmap xs1 ts1 !! x) with (Some t).
        2:{ symmetry. unfold to_vtmap. apply lookup_list_to_map_zip_Some;
            [typeclasses eauto|]. exists i. split_and!...
            intros. apply NoDup_lookup with (i:=i) in H2... lia. }
        rewrite msubst_term_id... unfold to_vtmap. rewrite dom_list_to_map_zip_L...
      ...
      + assert (x ∉ xs1) by (intros contra; apply (Hdisjoint x); auto).
        ...
        2:{ symmetry. unfold to_vtmap. apply lookup_list_to_map_zip_None;
            [typeclasses eauto|]... }
        simpl. replace (to_vtmap xs2 ts2 !! x) with (Some t)...
        symmetry. unfold to_vtmap. apply lookup_list_to_map_zip_Some; [typeclasses eauto|].
  ...
  Proof with auto.
    ...
    - simpl. destruct (decide (x ∈ xs1 ∨ x ∈ xs2)).
      2:{ apply Decidable.not_or in n as [].
          simpl. assert (to_vtmap xs1 ts1 !! x = @None term).
          { unfold to_vtmap. apply lookup_list_to_map_zip_None... }
          assert (to_vtmap xs2 ts2 !! x = @None term).
          { unfold to_vtmap. apply lookup_list_to_map_zip_None... }
      ...
      + pose proof (Hdisjoint x H).
        apply elem_of_list_lookup in H as [i ?].
        destruct (lookup_of_same_length_l ts1 H) as [t ?].
        assert (to_vtmap xs1 ts1 !! x = Some t).
        { unfold to_vtmap. apply lookup_list_to_map_zip_Some;
            [typeclasses eauto|]. exists i. split_and!... intros.
            apply NoDup_lookup with (i:=i) in H2... lia. }
        rewrite H2. rewrite msubst_term_id.
        2:{ unfold to_vtmap. rewrite dom_list_to_map_zip_L...
            clear dependent x. intros x ??. apply (Hfree2 _ H). apply elem_of_union_list.
            exists (term_fvars t). split... apply elem_of_list_fmap.
            apply elem_of_list_lookup_2 in H1. exists t... }
        replace (to_vtmap xs2 ts2 !! x) with (@None term).
        2:{ symmetry. unfold to_vtmap. apply lookup_list_to_map_zip_None... }
        simpl. rewrite H2...
      + assert (x ∉ xs1) by (intros contra; apply (Hdisjoint x); auto).
        replace (to_vtmap xs1 ts1 !! x) with (@None term).
...
Notation "t [ [ₜ xs \ ts ] ]" := (msubst_term t (to_vtmap xs ts))
                            (in custom formula at level 74, left associativity,
                              xs custom var_seq,
                              ts custom term_seq) : refiney_scope.
Notation "A [ [ xs \ ts ] ]" := (msubst A (to_vtmap xs ts))
                            (in custom formula at level 74, left associativity,
                              xs custom var_seq,
                              ts custom term_seq) : refiney_scope.
...
Section semantics.
  ...
  Definition teval_vtmap σ m mv :=
      dom mv = dom m ∧ ∀ x t v, m !! x = Some t → mv !! x = Some v → teval σ t v.

  Lemma teval_vtmap_total σ m :
    ∃ mv : gmap variable value, teval_vtmap σ m mv.
  Proof with auto.
    unfold teval_vtmap. induction m using map_ind.
  ...
  Lemma teval_vtmap_insert {σ m mv} x t v :
    teval σ t v →
    teval_vtmap σ m mv → teval_vtmap σ (<[x:=t]> m) (<[x:=v]> mv).
  Proof with auto.
    intros ? []. unfold teval_vtmap. split.
  ...
  Lemma teval_vtmap_delete {σ m mv} x :
    teval_vtmap σ m mv → teval_vtmap σ (delete x m) (delete x mv).
  Proof with auto.
    intros []. unfold teval_vtmap. split.
  ...
  Lemma teval_vtmap_det {σ} m mv mv' :
    teval_vtmap σ m mv →
    teval_vtmap σ m mv' →
    mv = mv'.
  Proof with auto.
  ...
  Proof with auto.
    intros A B H m ? <- σ. destruct (teval_vtmap_total σ m) as [mv ?].
  ...
  Proof with auto.
    intros.
    assert (fresh_var y (quant_msubst_fvars y A (to_vtmap xs ts)) = y).
    { apply fresh_var_id... }
  ...
  Local Lemma teval_vtmap_total_inv σ mv :
    ∃ m, teval_vtmap σ m mv.
  Proof with auto.
  ...
  Lemma teval_delete_state_vtmap_head σ t mv v :
    dom mv ## term_fvars t →
    teval (mv ∪ σ) t v ↔ teval σ t v.
  Proof with auto.
    destruct (teval_vtmap_total_inv σ mv) as [m Hm].
    intros. rewrite <- teval_msubst with (m:=m)...
    rewrite msubst_term_id... rewrite <- (proj1 Hm)...
  Qed.

  Lemma afeval_delete_state_vtmap_head σ af mv :
    dom mv ## af_fvars af →
  ...
  Lemma feval_delete_state_vtmap_head σ A mv :
  ...
  Lemma teval_vtmap_delete_state_vtmap_head {σ1 m mv} (σ2 : gmap variable value)  :
    dom σ2 ## vtmap_fvars m →
    teval_vtmap (σ2 ∪ σ1) m mv ↔ teval_vtmap σ1 m mv.
  Proof with auto.
    intros. unfold teval_vtmap. split; intros []; split; auto; intros.
    - unfold vtmap_fvars in H. set_unfold in H. setoid_rewrite elem_of_union_list in H.
      setoid_rewrite elem_of_list_fmap in H. simpl in H. specialize (H1 _ _ _ H2 H3).
      apply teval_delete_state_vtmap_head in H1... intros x' ??.
      apply H in H5 as []. exists (term_fvars t). split... exists (x, t). simpl.
      split... apply elem_of_map_to_list...
    - unfold vtmap_fvars in H. set_unfold in H. setoid_rewrite elem_of_union_list in H.
      setoid_rewrite elem_of_list_fmap in H. simpl in H. specialize (H1 _ _ _ H2 H3).
      apply teval_delete_state_vtmap_head... intros x' ??.
      apply H in H4 as []. exists (term_fvars t). split... exists (x, t). simpl.
      split... apply elem_of_map_to_list...
  Qed.
  ...
  Proof with auto.
    ...
    pose proof (teval_vtmap_total σ m) as [mv Hmv].
    apply teval_vtmap_delete with (x:=x) in Hmv as H1.
    pose proof (teval_total σ t) as [v Hv].
    apply (teval_vtmap_insert x t v Hv) in H1 as H2.
  ...
  Proof with auto.
    ...
    pose proof (teval_total σ t) as [v Hv].
    pose proof (teval_vtmap_total σ (m)) as [mv Hmv].
    apply teval_vtmap_delete with (x:=x) in Hmv as H2.
    apply (teval_vtmap_insert x _ _ Hv) in H2 as H3.
    rewrite feval_msubst with (mv:=<[x:=v]> (delete x mv))...
    rewrite feval_msubst with (mv:=delete x mv)...
    assert (Hv':=Hv).
    rewrite <- teval_delete_state_vtmap_head with (mv:=delete x mv) in Hv'...
    2: { rewrite (proj1 H2)... }
  ...
  Proof with auto.
    ...
    2:{ destruct H2 as []. split... intros. pose proof (H2 x0 t0 v0 H4 H5).
        pose proof (not_elem_of_vtmap_fvars _ _ H0). apply H7 in H4.
  ...
  Proof with auto.
    intros. unfold to_vtmap. simpl. rewrite (msubst_extract_l' A x t).
  ...
  Proof with auto.
    intros. unfold to_vtmap. repeat rewrite app_nil_r.
    simpl. rewrite (msubst_extract_r' A x t).
    - rewrite delete_insert... apply lookup_list_to_map_zip_None...
    - rewrite lookup_insert...
    - intros contra. apply elem_of_vtmap_fvars in contra as (y&t0&?&?).
  ...
  Proof with auto.
    intros. unfold to_vtmap. repeat rewrite app_nil_r. f_equiv.
  ...
  Proof with auto.
    ...
    - rewrite IHA1; [| set_solver]. rewrite IHA2; [| set_solver]...
    - simpl. generalize_fresh_var_for_msubst x A (to_vtmap xs ts) as x'.
      unfold to_vtmap in H4. rewrite dom_list_to_map_zip_L in H4...
      destruct H3.
  ...
  Proof with auto.
    ...
    opose proof (teval_vtmap_total σ (to_vtmap xs1 ts)) as [mv ?].
    opose proof (teval_vtmap_total σ _) as [mvx2t ?].
    rewrite feval_msubst with (mv:=mvx2t) by exact H2.
    rewrite feval_msubst with (mv:=mv).
    2:{ unfold to_vtmap in *. destruct H1 as [], H2 as [].
        split.
    ...
    2:{ destruct H1 as (?&?), H2 as (?&?). apply map_disjoint_dom. rewrite H1.
        rewrite H2. unfold to_vtmap. rewrite dom_list_to_map_zip_L...
        rewrite dom_list_to_map_zip_L...
        set_solver. }
    rewrite <- map_union_assoc. rewrite feval_delete_state_vtmap_head.
    2:{ unfold teval_vtmap in H2. destruct H2 as [-> _]. unfold to_vtmap.
        rewrite dom_list_to_map_zip_L... }
    rewrite feval_msubst with (mv:=mv)...
  Qed.

  Lemma msubst_term_msubst_term_eq t xs ts1 ts2 `{!OfSameLength xs ts1} `{!OfSameLength xs ts2} :
    NoDup xs →
    <! t [[ₜ *xs \ *ts1 ]][[ₜ *xs \ *ts2 ]] !> ≡
      <! t [[ₜ *xs \ *$((λ t, <! t[[ₜ *xs \ *ts2]] !>) <$> ts1)]] !>.
  Proof with auto.
    intros Hnodup σ v.
    opose proof (teval_vtmap_total σ _) as [mv1 ?].
    rewrite teval_msubst with (mv:=mv1) by exact H.
    opose proof (teval_vtmap_total _ _) as [mv2 ?].
    rewrite teval_msubst with (mv:=mv2) by exact H0.
    opose proof (teval_vtmap_total _ _) as [mv3 ?].
    rewrite teval_msubst with (mv:=mv3) by exact H1.
    ...
    2:{ enough (x ∉ dom mv3 ∧ x ∉ dom (mv2 ∪ mv1)).
        ...
        - rewrite dom_union. rewrite H, H0, H1. unfold to_vtmap.
    ...
    { apply elem_of_dom. rewrite H0. unfold to_vtmap. rewrite dom_list_to_map_zip_L...
      set_solver. }
    rewrite lookup_union_l'... destruct H5 as [v2 ?].
    assert (is_Some (mv3 !! x)) as (v3&?).
    { apply elem_of_dom. rewrite H1. unfold to_vtmap. rewrite dom_list_to_map_zip_L;
    ...
    - apply H3 with (x:=x)... unfold to_vtmap. apply lookup_list_to_map_zip_Some...
      exists i. split_and!... intros. eapply NoDup_lookup with (i:=i) in H9... lia.
    - specialize (H4 x <! t1 [[ₜ *xs \ *ts2]] !> v3).
      rewrite <- teval_msubst; [apply H4| split; auto]... unfold to_vtmap.
  ...
  Lemma msubst_term_msubst_term_eq_cancel t xs ts1 ts2
    `{!OfSameLength xs ts1} `{!OfSameLength xs ts2} :
    list_to_set xs ## ⋃ (term_fvars <$> ts1) →
    NoDup xs →
    <! t [[ₜ *xs \ *ts1 ]][[ₜ *xs \ *ts2 ]] !> ≡
      <! t [[ₜ *xs \ *ts1 ]] !>.
  Proof with auto.
    intros. rewrite msubst_term_msubst_term_eq... f_equiv. apply map_eq. intros x.
    unfold to_vtmap.
    destruct
      (list_to_map (zip xs ((λ t0, msubst_term t0 (list_to_map (zip xs ts2))) <$> ts1)) !! x)
        as [t2|]
        eqn:E1;
      destruct (list_to_map (zip xs ts1) !! x) as [t1|] eqn:E2...
      - apply lookup_list_to_map_zip_Some in E2 as (i&?&?&_)...
        assert (H3:=H1). apply (lookup_of_same_length_l ts2) in H3 as [t2' ?].
        apply lookup_list_to_map_zip_Some in E1 as (j&?&?&_)...
        2:{ typeclasses eauto. }
        apply list_lookup_fmap_Some in H5 as (t1'&?&?).
        apply NoDup_lookup with (j:=j) in H1... subst j. rewrite H5 in H2. inversion H2.
        subst t1'. subst t2. f_equal. fold (to_vtmap xs ts2). rewrite msubst_term_non_free...
        clear dependent x. intros x ??. apply (H x)... apply elem_of_union_list.
        exists (term_fvars t1). split... apply elem_of_list_fmap. exists t1. split...
        apply elem_of_list_lookup_2 in H5...
      - apply lookup_list_to_map_zip_Some in E1 as (?&?&_)...
        2:{ typeclasses eauto. }
        apply lookup_list_to_map_zip_None in E2... apply elem_of_list_lookup_2 in H1. done.
      - apply lookup_list_to_map_zip_Some in E2 as (?&?&_)...
        apply lookup_list_to_map_zip_None in E1...
        2:{ typeclasses eauto. }
        apply elem_of_list_lookup_2 in H1. done.
  Qed.
  ...
  Lemma msubst_term_msubst_term_disj t xs1 ts1 xs2 ts2 `{!OfSameLength xs1 ts1} `{!OfSameLength xs2 ts2} :
    ...
    NoDup xs2 →
    list_to_set xs1 ## ⋃ (term_fvars <$> ts2) →
    <! t [[ₜ *xs1 \ *ts1 ]][[ₜ *xs2 \ *ts2 ]] !> ≡
      <! t [[ₜ *xs2 \ *ts2 ]] [[ₜ *xs1 \ *$((λ t, <! t[[ₜ *xs2 \ *ts2]] !>) <$> ts1) ]] !>.
  Proof with auto.
    intros Hdisjoint Hnodup1 Hnodup2 Hfree σ v.
    opose proof (teval_vtmap_total σ _) as [mv1 ?].
    rewrite teval_msubst with (mv:=mv1) by exact H.
    opose proof (teval_vtmap_total _ _) as [mv2 ?].
    rewrite teval_msubst with (mv:=mv2) by exact H0.
    opose proof (teval_vtmap_total _ _) as [mv3 ?].
    rewrite teval_msubst with (mv:=mv3) by exact H1.
    opose proof (teval_vtmap_total _ _) as [mv4 ?].
    rewrite teval_msubst with (mv:=mv4) by exact H2.
    ...
    2:{ apply Decidable.not_or in n as []. enough (x ∉ dom (mv2 ∪ mv1) ∧ x ∉ dom (mv4 ∪ mv3)).
        ...
        - do 2 rewrite dom_union. rewrite H, H0, H1, H2. unfold to_vtmap.
    ...
    - apply Hdisjoint in H7 as ?.
      ...
      { apply elem_of_dom. rewrite H1. unfold to_vtmap. clear dependent mv1 mv2 mv3 mv4.
        rewrite dom_list_to_map_zip_L; [set_solver|]. typeclasses eauto. }
      assert (mv4 !! x = None).
      { apply not_elem_of_dom. rewrite H2. unfold to_vtmap. clear dependent mv1 mv2 mv3 mv4.
      ...
      + apply H4 with (x:=x)... unfold to_vtmap. apply lookup_list_to_map_zip_Some...
        exists i. split_and!... intros. eapply NoDup_lookup with (i:=i) in H13... lia.
      + specialize (H5 x <! t' [[ₜ *xs2 \ *ts2 ]] !> v3).
        rewrite <- teval_msubst; [apply H5| split; auto]... unfold to_vtmap.
        apply lookup_list_to_map_zip_Some.
        { apply of_same_length_fmap_r. }
        exists i. split_and!...
    ...
    - assert (x ∉ xs1) by (intros contra; apply (Hdisjoint x); auto).
      ...
      { apply elem_of_dom. rewrite H. unfold to_vtmap. rewrite dom_list_to_map_zip_L...
        clear dependent mv1 mv2 mv3 mv4. set_solver. }
      assert (is_Some (mv4 !! x)).
      { apply elem_of_dom. rewrite H2. unfold to_vtmap. clear dependent mv1 mv2 mv3 mv4.
        rewrite dom_list_to_map_zip_L; [set_solver|]. typeclasses eauto. }
      assert (mv2 !! x = None).
      { apply not_elem_of_dom. rewrite H0. unfold to_vtmap. clear dependent mv1 mv2 mv3 mv4.
        rewrite dom_list_to_map_zip_L; [set_solver|]... }
      ...
      + ospecialize (H6 x t' v4 _ _)...
        { unfold to_vtmap. apply lookup_list_to_map_zip_Some... exists i. split_and!...
          intros. apply NoDup_lookup with (i:=i) in H13... lia. }
        rewrite <- teval_delete_state_vtmap_head; [exact H6|].
        rewrite H1. unfold to_vtmap. rewrite dom_list_to_map_zip_L.
        2:{ typeclasses eauto. }
