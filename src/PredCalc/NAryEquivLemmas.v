...
Section n_ary_lemmas.
  ...
  Proof with auto.
    ...
    2: { simpl. set_solver. }
    pose proof (teval_vtmap_total σ (to_vtmap (x::xs) (t::ts))) as (mv&?).
    rewrite feval_msubst with (mv:=mv)...
    pose proof (teval_vtmap_total σ (to_vtmap (x::xs) ((TConst vt)::ts))) as (mv'&?).
    ...
    eapply teval_vtmap_zip_cons_inv in H5 as (v&mv0&?&->&?&?)...
    eapply teval_vtmap_zip_cons_inv in H6 as (v'&mv1&?&->&?&?)...
    inversion H6. subst v0. subst v'. pose proof (teval_det t vt v Hvt H2) as ->.
    pose proof (teval_vtmap_det _ _ _ H5 H8) as ->...
  Qed.
