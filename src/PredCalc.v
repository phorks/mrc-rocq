...
Section semantics.
  ...
  Lemma feval_exists_equiv {σ1 σ2 x1 x2 τ A1 A2} b:
    (∀ v, v ∈ τ →
          feval (<[x1:=v]> σ1) A1 true ↔ feval (<[x2:=v]> σ2) A2 true) →
    ((∃ v, v ∈ τ) →
     ∀ v, v ∈ τ →
          feval (<[x1:=v]> σ1) A1 false ↔ feval (<[x2:=v]> σ2) A2 false) →
    (value_ty_is_empty τ →
  ...
  Proof with eauto.
    ...
    - apply FEval_Exists_True. destruct H7 as [v []]. apply (H v) in H4...
    - apply FEval_Exists_False... intros. apply H0...
    - apply FEval_Exists_False_Empty... apply H1...
    - apply FEval_Exists_True. destruct H7 as [v []]. apply (H v) in H4...
    - apply FEval_Exists_False... intros. apply H0...
    - apply FEval_Exists_False_Empty... apply H1...
  Qed.
